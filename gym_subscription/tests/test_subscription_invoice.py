...
from odoo.addons.gym_subscription.tests.common_sale_subscription import TestSubscriptionCommon, UncatchableException
...
class TestSubscriptionInvoice(TestSubscriptionCommon):
    ...
    def test_sale_subscription_post_invoice(self):
        ...
        with patch('odoo.addons.gym_subscription.models.sale_order_line.SaleOrderLine._reset_subscription_quantity_post_invoice', patched_reset), freeze_time("2021-01-01"):
            sub = self.subscription
            sub.action_confirm()
            self.env['sale.order']._cron_recurring_create_invoice()
    ...
    def test_failing_sub_invoice_flag(self):
        ...
        try:
            with patch(
                    'odoo.addons.gym_subscription.models.sale_order.SaleOrder._process_invoices_to_send',
