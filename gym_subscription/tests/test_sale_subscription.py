...
from odoo.addons.gym_subscription.models.sale_order import SaleOrder
...
class TestSubscription(TestSubscriptionCommon, MockEmail):
    ...
    def test_subscription_change_partner(self):
        ...
        with patch('odoo.addons.gym_subscription.models.sale_order.SaleOrder.action_confirm', _action_confirm_mock):
    ...
    def test_reopen(self):
        ...
        with freeze_time("2023-03-05"):
            close_reason_id = self.env.ref('gym_subscription.close_reason_1').id
    ...
    def test_close_reason_end_of_contract(self):
        ...
        self.assertEqual(sub.close_reason_id.id, self.env.ref('gym_subscription.close_reason_end_of_contract').id)
    ...
    def test_cron_recurring_send_payment_reminder_failure(self):
        ...
        with self.mock_mail_gateway():
            with freeze_time("2024-04-17"):
                self.env.ref('gym_subscription.send_payment_reminder').lastcall = False
                self.env['sale.order']._cron_recurring_send_payment_reminder()
                self.assertEqual(self.subscription.last_reminder_date, fields.Date.today())
                self.assertEqual(len(self._new_mails), 1)
            with freeze_time("2024-04-25"):
                ...
                self.env.ref('gym_subscription.send_payment_reminder').lastcall = datetime.datetime(2024, 4, 23, 0, 0, 0)
                self.env['sale.order']._cron_recurring_send_payment_reminder()
                self.assertEqual(self.subscription.last_reminder_date, fields.Date.today())
                self.assertEqual(len(self._new_mails), 2)
