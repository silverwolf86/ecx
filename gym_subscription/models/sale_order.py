...
class SaleOrder(models.Model):
    ...
    def _compute_recurring_details(self):
        ...
        if subscription_orders.ids:
            for so in subscription_orders:
                ...
                so.recurring_details = self.env['ir.qweb']._render('gym_subscription.recurring_details', {'rendering_values': rendering_values})
    ...
    def _track_subtype(self, init_values):
        ...
        if 'subscription_state' in init_values:
            return self.env.ref('gym_subscription.subtype_state_change')
        if self.is_subscription and 'next_invoice_date' in init_values and self.env.context.get('subscription_pause'):
            return self.env.ref('gym_subscription.subtype_resume_and_pause_subscription')
    ...
    def _confirm_renewal(self):
        ...
        for renew in self:
            ...
            renew_close_reason_id = self.env.ref('gym_subscription.close_reason_renew')
            end_of_contract_reason_id = self.env.ref('gym_subscription.close_reason_end_of_contract')
    ...
    def _get_associated_so_action(self):
        return {
            ...
            "views": [[self.env.ref('gym_subscription.sale_subscription_view_tree').id, "list"],
                      [self.env.ref('gym_subscription.sale_subscription_primary_form_view').id, "form"],
    ...
    def open_subscription_history(self):
        ...
        action = {
            "type": "ir.actions.act_window",
            "res_model": "sale.order",
            "views": [[self.env.ref('gym_subscription.sale_subscription_quotation_tree_view').id, "list"],
                      [self.env.ref('gym_subscription.sale_subscription_primary_form_view').id, "form"]]
        }
    ...
    def open_subscription_renewal(self):
        ...
        if len(renewal) == 1:
            ...
            action['views'] = [(self.env.ref('gym_subscription.sale_subscription_primary_form_view').id, 'form')]
        else:
    ...
    def action_sale_order_log(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("gym_subscription.sale_order_log_analysis_action")
    ...
    def _get_order_digest(self, origin='', template='gym_subscription.sale_order_digest', lang=None):
        self.ensure_one()
    ...
    def set_close(self, close_reason_id=None, renew=False):
        ...
        if not close_reason_id:
            renew_close_reason_id = self.env.ref('gym_subscription.close_reason_renew').id
            end_of_contract_reason_id = self.env.ref('gym_subscription.close_reason_end_of_contract').id
            close_reason_unknown_id = self.env.ref('gym_subscription.close_reason_unknown').id
        for sub in self:
    ...
    def _get_portal_return_action(self):
        ...
        if self.is_subscription:
            return self.env.ref('gym_subscription.sale_subscription_action')
        else:
    ...
    def _handle_subscription_payment_failure(self, invoice, transaction):
        ...
        reminder_mail_template = self.env.ref('gym_subscription.email_payment_reminder', raise_if_not_found=False)
        close_mail_template = self.env.ref('gym_subscription.email_payment_close', raise_if_not_found=False)
        invoice.unlink()
        ...
        for order in self:
            ...
            if close_contract:
                ...
                order.set_close(close_reason_id=order.env.ref('gym_subscription.close_reason_auto_close_limit_reached').id)
            else:
    ...
    def _subscription_launch_cron_parallel(self, batch_size):
        self.env.ref('gym_subscription.account_analytic_cron_for_invoice')._trigger()
    ...
    def _create_recurring_invoice(self, batch_size=30):
        ...
        grouped_invoice = self.env['ir.config_parameter'].get_param('gym_subscription.invoice_consolidation', False)
    ...
    def _get_traceback_body(self, exc, body):
        if not str2bool(self.env['ir.config_parameter'].sudo().get_param('gym_subscription.full_mail_traceback')):
    ...
    def _cron_subscription_expiration(self):
        ...
        expired_close_reason = self.env.ref('gym_subscription.close_reason_auto_close_limit_reached')
        unpaid_close_reason = self.env.ref('gym_subscription.close_reason_unpaid_subscription')
    ...
    def _get_name_portal_content_view(self):
        return 'gym_subscription.subscription_portal_content' if self.is_subscription else super()._get_name_portal_content_view()
    ...
    def _subscription_reminder_parameters(self):
        ...
        reminder_cron = self.env.ref('gym_subscription.send_payment_reminder', raise_if_not_found=False)
    ...
    def _cron_recurring_send_payment_reminder(self):
        ...
        for subscription in all_subscriptions:
            ...
            if close_contract and subscription.next_invoice_date not in parameters['next_invoice_dates']:
                ...
                subscription.set_close(close_reason_id=self.env.ref('gym_subscription.close_reason_auto_close_limit_reached').id)
            else:
                # Send the reminder
