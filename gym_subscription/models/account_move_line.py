...
class AccountMoveLine(models.Model):
    ...
    # These two fields are normally provided by the Enterprise `account_accountant` module
    # (deferred revenue/expense management). They are declared here instead since this module
    # relies on them to track the invoiced period of each recurring line, independently of
    # any deferred revenue accounting automation.
    deferred_start_date = fields.Date(string="Start Date", index="btree_not_null", copy=False)
    deferred_end_date = fields.Date(string="End Date", index="btree_not_null", copy=False)
