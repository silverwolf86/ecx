...
from odoo.addons.ecx_edi.models.account_move import L10N_EC_VAT_SUBTAXES
...
from odoo.addons.ecx_edi.models.xml_utils import (
    NS_MAP,
...
class AccountEdiFormat(models.Model):
    ...
    def _check_move_configuration(self, move):
        ...
        if not move.company_id.sudo().ecx_edi_certificate_id and not move.company_id._l10n_ec_is_demo_environment():
    ...
    def _l10n_ec_generate_xml(self, move):
        ...
        if move.journal_id.l10n_ec_withhold_type:  # withholds
            doc_type = 'withhold'
            template = 'ecx_edi.withhold_template'
            move_info.update(move._l10n_ec_get_withhold_edi_data())
        else:  # invoices
            doc_type = move.l10n_latam_document_type_id.internal_type
            template = {
                'credit_note': 'ecx_edi.credit_note_template',
                'debit_note': 'ecx_edi.debit_note_template',
                'invoice': 'ecx_edi.invoice_template',
                'purchase_liquidation': 'ecx_edi.purchase_liquidation_template',
            }[doc_type]
    ...
    def _l10n_ec_generate_signed_xml(self, company_id, xml_node_or_string):
        if company_id._l10n_ec_is_demo_environment():  # unless we're in a test environment without certificate
            ...
            certificate_sudo = company_id.sudo().ecx_edi_certificate_id
            ...
            qweb_values.update({
                ...
                'x509_issuer_description': certificate_sudo._ecx_edi_get_issuer_rfc_string(),
            ...
            signature_str = self.env['ir.qweb']._render('ecx_edi.ec_edi_signature', qweb_values)
    ...
    def _l10n_ec_create_authorization_file_new(self, company_id, xml_string, authorization_number, authorization_date):
        ...
        xml_response = self.env['ir.qweb']._render('ecx_edi.authorization_template', xml_values)
