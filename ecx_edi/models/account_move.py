...
from odoo.addons.ecx_edi.models.account_tax import L10N_EC_TAXSUPPORTS
...
class AccountMove(models.Model):
    ...
    def action_print_pdf(self):
        ...
        return self.env.ref('ecx_edi.ecx_edi_withhold').report_action(self.id)
    ...
    def l10n_ec_action_send_withhold(self):
        ...
        self.ensure_one()
        template = self.env.ref('ecx_edi.email_template_edi_withhold')
        report = self.env.ref('ecx_edi.ecx_edi_withhold')
        return {
    ...
    def _get_name_invoice_report(self):
        ...
        if self.l10n_latam_use_documents and self.country_code == 'EC':
            if (self.move_type in ('out_invoice', 'out_refund') and doc_type_code in ['01', '04', '05', '41']) \
                ...
                return 'ecx_edi.report_invoice_document'
    ...
    def _get_invoice_pdf_proforma(self):
        ...
        content, report_type = self.env['ir.actions.report']._pre_render_qweb_pdf('ecx_edi.ecx_edi_withhold', self.ids, data={'proforma': True})
        content_by_id = self.env['ir.actions.report']._get_splitted_report('ecx_edi.ecx_edi_withhold', content, report_type)
        return {
            'filename': filename,
            'filetype': 'pdf',
    ...
    def _get_edi_decoder(self, file_data: dict, new=False):
        ...
        if file_data['import_file_type'] == 'l10n_ec.factura':
            return {
                'priority': 20,
                'decoder': self._ecx_edi_import_bill,
            }
        return super()._get_edi_decoder(file_data, new=new)

    def _ecx_edi_import_bill(self, bill, file_data: dict, new: bool = False) -> bool | None:
        ...
        with bill._get_edi_creation() as bill:
            ...
            self._ecx_edi_import_bill_fill_move_line(tree.findall('.//detalles//detalle'), bill)
            bill.write(bill_vals)

    def _ecx_edi_import_bill_fill_move_line(self, line_nodes, bill) -> None:
