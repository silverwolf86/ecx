...
class StockPicking(models.Model):
    ...
    ecx_edi_document_number = fields.Char(
    ...
    ecx_edi_status = fields.Selection(
        selection=[
            ('to_send', "To Send"),
            ('sent', "Sent"),
            ('to_cancel', "To Cancel"),
    ...
    @api.depends('state', 'l10n_ec_is_delivery_guide', 'ecx_edi_status')
    def _compute_l10n_ec_allow_send_edi(self):
        ...
        for stock_picking in self:
            stock_picking.l10n_ec_allow_send_edi = stock_picking.state == 'done' and \
                stock_picking.l10n_ec_is_delivery_guide and not stock_picking.ecx_edi_status
    ...
    def l10n_ec_action_create_delivery_guide(self):
        ...
        """
        self._check_company()
        self._ecx_edi_validations()
        account_edi_format_model = self.env['account.edi.format']
        for record in self:
            # == Generate a document number ==
            wh_id = record.picking_type_id.warehouse_id
            if not record.ecx_edi_document_number:
                record.ecx_edi_document_number = f'{wh_id.l10n_ec_entity}-{wh_id.l10n_ec_emission}-{wh_id.l10n_ec_delivery_number_sequence_id.next_by_id()}'
            record.l10n_ec_authorization_number = record._l10n_ec_get_authorization_number()

            # == Create and sign the delivery guide ==
            edi_str = record._ecx_edi_create_delivery_guide()
            signed_edi_str = account_edi_format_model._l10n_ec_generate_signed_xml(record.company_id, edi_str)
            record.write({
                'ecx_edi_status': 'to_send',
                'ecx_edi_content': b64encode(signed_edi_str.encode()),
    ...
    def l10n_ec_send_delivery_guide_to_send(self):
        ...
        This method filters the stock picking records to find those with an 'ecx_edi_status' of 'to_send'.
        ...
        try:
            ...
            return
        for record in self.filtered(lambda x: x.ecx_edi_status == 'to_send'):
            signed_edi_str = b64decode(record.ecx_edi_content).decode()
            ...
            if call_status == 'error' and errors:
                ...
                record.write({
                    'l10n_ec_delivery_guide_error': False,
                    'ecx_edi_status': 'sent',
                    'ecx_edi_content': attachment.datas,
                })
    ...
    def l10n_ec_send_delivery_guide_to_partner(self):
        ...
        template = self.env.ref('ecx_edi_stock.email_template_edi_delivery_guide')
        xml_attachment = self.env['ir.attachment'].create({
            'name': f'GuíRe {self.ecx_edi_document_number}.xml',
            'type': 'binary',
            'datas': self.ecx_edi_content,
            'res_model': self._name,
            'res_id': self.id,
            'mimetype': 'application/xml',
        })
        pdf_action = self.env.ref('ecx_edi_stock.action_delivery_guide_report_pdf')
        pdf_content, __ = self.env['ir.actions.report'].sudo()._render_qweb_pdf(pdf_action, self.id)
        pdf_attachment = self.env['ir.attachment'].create({
            'name': f'GuíRe {self.ecx_edi_document_number}.pdf',
            'type': 'binary',
    ...
    def l10n_ec_action_download_delivery_guide(self):
        ...
        return {
            ...
            'url': '/web/content/stock.picking/%s/ecx_edi_content?download=true&filename=%s' % (
                self.id, f'GuíRe {self.ecx_edi_document_number}.xml',
            ),
            'target': 'self',
        }

    def _ecx_edi_create_delivery_guide(self):
        """
        Create the XML request content for the delivery guide.
        """
        self.ensure_one()
        values = self._ecx_edi_get_delivery_guide_values()
        xml_string = self.env['ir.qweb']._render(
            'ecx_edi_stock.sri_delivery_guide',
            values
        )
        xml_content = cleanup_xml_node(xml_string)
        return xml_content

    def _ecx_edi_get_delivery_guide_values(self):
        '''
        ...
        return {
            ...
            'sequence': self.ecx_edi_document_number.split('-')[-1],
    ...
    def _l10n_ec_get_authorization_number(self):
        ...
        sequential = self.ecx_edi_document_number.split('-')[-1]
    ...
    def l10n_ec_send_delivery_guide_to_cancel(self):
        ...
        try:
            ...
            if not self.env.context.get('cron_skip_connection_errs'):
                raise UserError(_('Some of these electronic documents are already being processed.')) from None
            return
        for record in self.filtered(lambda x: x.ecx_edi_status == 'to_cancel'):
    ...
    def _l10n_ec_send_xml_to_authorize(self, picking, xml_string):
        ...
        if auth_num and auth_date:
            ...
            attachment = self.env['ir.attachment'].create({
                'name': f'GuíRe {picking.ecx_edi_document_number}.xml',
                'res_id': picking.id,
    ...
    def _l10n_ec_cron_send_delivery_guide_to_sri(self):
        ...
        Send the delivery guide for the stock picking records that are pending to be sent.
        """
        for record in self.search([('ecx_edi_status', 'in', ('to_send', 'to_cancel'))]):
            if record.ecx_edi_status == 'to_cancel':
    ...
    def _l10n_ec_get_delivery_guide_additional_info(self):
        ...
        """
        return {
            'Referencia': f'GuíRe {self.ecx_edi_document_number or "/"}',
